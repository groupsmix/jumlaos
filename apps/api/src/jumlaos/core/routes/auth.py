...
from jumlaos.core.models import Membership, MembershipStatus, OtpCode, Role, User
...
class OtpRequestResponse(BaseModel):
    phone: str
    # Dev-only: returns the OTP so automated tests don't need a WhatsApp channel.
...
async def otp_request(
    ...
    return OtpRequestResponse(
        phone=phone,
        dev_otp=code if settings.is_dev else None,
...
async def refresh_tokens(
    ...
    new_refresh = issue_refresh_token(user_id=user_id)
...
async def switch_business(
    ...
    refresh = issue_refresh_token(user_id=ctx.user_id)
    _set_auth_cookies(response, access=access, refresh=refresh)
